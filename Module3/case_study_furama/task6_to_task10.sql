...
from dich_vu dv
         ...
         join hop_dong hd on hd.ma_dich_vu = dv.ma_dich_vu
where (select year (hd.ngay_lam_hop_dong)) = 2020 and (
select year (hd.ngay_lam_hop_dong)) <> 2021;
;
...
select month (hd.ngay_lam_hop_dong) as thang, count (hd.ma_hop_dong) as so_luong_khach_dat_phong
from
    hop_dong hd
group by thang
order by thang;
